...
function ReviewsPage({ onNavigate }) {
  ...
  const [newReview, setNewReview] = useState({ name: '', rating: 5, text: '', email: '', photo: null });
  ...
  const handleSubmitReview = (e) => {
    ...
    setReviewsList([
      { id: Date.now(), name: newReview.name, rating: newReview.rating, date: dateStr, text: newReview.text, photo: newReview.photo ? URL.createObjectURL(newReview.photo) : null },
      ...reviewsList
    ]);
    setSubmitted(true);
    setNewReview({ name: '', rating: 5, text: '', email: '', photo: null });
  };
              ...
              <div className="testimonials-list">
                {reviewsList.map((rev) => (
                  <div key={rev.id} className="testimonial-card">
                    ...
                    {rev.photo && (
                      <div className="tc-photo" style={{marginTop: '10px'}}>
                        <img src={rev.photo} alt="Customer upload" style={{width: '100px', height: '100px', objectFit: 'cover', borderRadius: '8px'}} />
                      </div>
                    )}
                  </div>
                ))}
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                           if (e.target.files.length > 0) {
                             setNewReview(prev => ({ ...prev, photo: e.target.files[0] }));
                           }
                        }}
                      />
                    </div>
